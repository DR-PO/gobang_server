...
int main() {
	...
	connectionPool->init("localhost","gobang", "123456", "GoBangServer",  3306, 10);
	//互斥量 初始化
...
//		std:: cout << ready << "  " << errno << '\n';
		...
		//如果超时了 处理超时事件
		if(timeout){
			timeList.tick(); // 处理所有过期定时器
			alarm(TIMESLOT); // 重新设置定时器
			timeout = false; // 重置标志
			printServerMsg(); // 打印服务器状态
		}
	}
	close();
...
void initNet(){
	...
	//定时内容 统一事件源机制，将信号事件转换为I/O事件
	ret = socketpair(PF_UNIX, SOCK_STREAM, 0, pipefd);
	if(ret == -1) myUtils::print_sys_error("socketpair error");
	myUtils::setNonBlock(pipefd[1]); //将管道写端设置为非阻塞，防止信号处理函数中阻塞
	myUtils::fdAdd(ep_fd, pipefd[0], false); // 添加到epoll监听
	addSig(SIGALRM); // 定时器信号
	addSig(SIGTERM); // 终止信号
	addSig(SIGINT); // 中断信号Ctrl+C
	alarm(TIMESLOT); //设置定时器，每10秒触发一次SIGALRM信号
}
